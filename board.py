	...
	def get_play_index(self, play_obj):
		if play_obj["movetype"] == "p":
			...
			if play_obj["direction"] == "up":
				for index, item in enumerate(self.generate_move_arrays(y, self.board_size,False)):
				...
				offset2 = moves_up
				for index, item in enumerate(self.generate_move_arrays(self.board_size -1 - x, self.board_size,False)):
					if item == play_obj["order"]:
						offset2 += index -1
						break
				...
				for index, item in enumerate(self.generate_move_arrays(self.board_size -1 - y, self.board_size,False)):
					if item == play_obj["order"]:
						offset2 += index -1
						break
				...
				for index, item in enumerate(self.generate_move_arrays(x, self.board_size,False)):
					if item == play_obj["order"]:
						offset2 += index -1
						break
